...
def generate_out_img_name(config):
    ...
    if 'reconstruct_script' in config:
        ...
        suffix = f'_o_{config["optimizer"]}_i_{config["init_method"]}_h_{str(config["height"])}_m_{config["model"]}_cw_{config["content_weight"]}_sw_{config["style_weight"]}_tv_{config["tv_weight"]}_layers_{config["layers"]}{config["img_format"][1]}'
...
def prepare_model(model, layers, device):
    ...
    if model == 'vgg16':
        ...
        model = Vgg19(layers, requires_grad=False, show_progress=True)
    elif model == 'resnet50':
        model = Resnet50(layers, requires_grad=False, show_progress=True)
    else:
